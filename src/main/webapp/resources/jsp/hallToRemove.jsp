...
<table>
    ...
    <%--<table>--%>
        <tbody>
        <tr><th>ID</th><th>Name</th><th>Quantity Of Rows</th><th>Places In Row</th>
            <c:if test="${sessionScope.action ne 'allHalls'}">
                <th>Select</th>
            </c:if>
        </tr>
        <c:forEach items="${requestScope.hallList}" var="hall">
            <tr><td>${hall.id}</td><td>${hall.name}</td><td>${hall.quantityOfRows}</td><td>${hall.placesInRow}</td>
                <c:if test="${sessionScope.action ne 'allHalls'}">
...
<br><br>
<c:if test="${sessionScope.action eq 'allHalls'}">
